				...
				if(!is_numeric($devid)){
					...
					if(mysqli_num_rows($result) == 1){
						...
						if(password_verify($authkey, $row["auth_key"])){
							...
							$stmt = $conn->prepare("UPDATE passwords SET password = ?, pass_key = ?, pass_iv = ? WHERE user_name = ? AND unique_id = ? AND url = ?");
							$stmt->bind_param("sssss", $encrypted_pass, $pass_key, $pass_iv, $uname, $uid, $url);
							$stmt->execute();
							$stmt->close();
