<?php
	...
	if(isset($_POST["unique_id"]) && isset($_POST["auth_key"]) && isset($_POST["device_id"]) && isset($_POST["url"]) && isset($_POST["user_name"])){
		...
		if(empty($uid) || empty($authkey) || empty($devid) || empty($url) || empty($uname)){
				...
				if(!is_numeric($devid)){
					...
					if(mysqli_num_rows($result) == 1){
						...
						if(password_verify($authkey, $row["auth_key"])){
							$stmt = $conn->prepare("DELETE FROM passwords WHERE user_name = ?");
